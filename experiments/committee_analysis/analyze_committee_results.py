...
# --- 1. CONFIGURATION ---
# UPDATED: Input from our new balanced judging script
INPUT_CSV = "committee_judged_results_balanced_v2.csv" 
# UPDATED: New output plot
OUTPUT_PLOT_FILE = "mps_distribution_histogram_v2.png" 
...
if __name__ == "__main__":
    try:
        ...
        print(f"Error: {INPUT_CSV} not found. Run 'run_committee_judging.py' first.")
        sys.exit(1)
    ...
    # Ensure total_models is not zero to avoid division error
    df_mps = df_mps[df_mps['total_models'] > 0]
    ...
    # Define bins
    bins = np.arange(0, 1.2, 0.1) # Bins [0.0, 0.1, 0.2, ..., 1.1]
    ...
    sns.histplot(
        ...
        kde=False, 
        color='salmon', 
        label=f"Malicious Prompts (n={len(df_malicious)})"
    )
    ...
    sns.histplot(
        ...
        kde=False, 
        color='skyblue', 
        label=f"Benign Prompts (n={len(df_benign)})"
    )
